...
public abstract class AbstractMetaKeyBean extends
		com.antlersoft.android.dbimpl.IdImplementationBase implements IMetaKey {
	...
	public static String GEN_CREATE = "CREATE TABLE META_KEY ("
			+ "_id INTEGER PRIMARY KEY AUTOINCREMENT," + "METALISTID INTEGER,"
			+ "KEYDESC TEXT," + "METAFLAGS INTEGER," + "MOUSECLICK INTEGER,"
			+ "MOUSEBUTTONS INTEGER," + "KEYSYM INTEGER," + "SHORTCUT TEXT"
			+ ")";
	...
	public android.content.ContentValues Gen_getValues() {
		...
		values.put(GEN_FIELD_MOUSEBUTTONS,
				Integer.toString(this.gen_mouseButtons));
	...
	public void Gen_populate(android.database.Cursor cursor, int[] columnIndices) {
		...
		if (columnIndices[GEN_ID_KEYDESC] >= 0
				&& !cursor.isNull(columnIndices[GEN_ID_KEYDESC])) {
			gen_keyDesc = cursor.getString(columnIndices[GEN_ID_KEYDESC]);
		}
		if (columnIndices[GEN_ID_METAFLAGS] >= 0
				&& !cursor.isNull(columnIndices[GEN_ID_METAFLAGS])) {
			gen_metaFlags = cursor.getInt(columnIndices[GEN_ID_METAFLAGS]);
		}
		if (columnIndices[GEN_ID_MOUSECLICK] >= 0
				&& !cursor.isNull(columnIndices[GEN_ID_MOUSECLICK])) {
			gen_mouseClick = (cursor.getInt(columnIndices[GEN_ID_MOUSECLICK]) != 0);
		}
		if (columnIndices[GEN_ID_MOUSEBUTTONS] >= 0
				&& !cursor.isNull(columnIndices[GEN_ID_MOUSEBUTTONS])) {
			gen_mouseButtons = cursor
					.getInt(columnIndices[GEN_ID_MOUSEBUTTONS]);
		}
		if (columnIndices[GEN_ID_KEYSYM] >= 0
				&& !cursor.isNull(columnIndices[GEN_ID_KEYSYM])) {
			gen_keySym = cursor.getInt(columnIndices[GEN_ID_KEYSYM]);
		}
		if (columnIndices[GEN_ID_SHORTCUT] >= 0
				&& !cursor.isNull(columnIndices[GEN_ID_SHORTCUT])) {
	...
	public void Gen_populate(android.content.ContentValues values) {
		...
		gen_metaFlags = values.getAsInteger(GEN_FIELD_METAFLAGS);
		gen_mouseClick = (values.getAsInteger(GEN_FIELD_MOUSECLICK) != 0);
		gen_mouseButtons = values.getAsInteger(GEN_FIELD_MOUSEBUTTONS);
		gen_keySym = values.getAsInteger(GEN_FIELD_KEYSYM);
