...
public class MostRecentBean extends
	...
	public static String GEN_CREATE = "CREATE TABLE MOST_RECENT ("
			...
			+ "CONNECTION_ID INTEGER," + "SHOW_SPLASH_VERSION INTEGER,"
			+ "TEXT_INDEX INTEGER" + ")";
	...
	public android.content.ContentValues Gen_getValues() {
		...
		values.put(GEN_FIELD_CONNECTION_ID,
				Long.toString(this.gen_CONNECTION_ID));
		values.put(GEN_FIELD_SHOW_SPLASH_VERSION,
				Long.toString(this.gen_SHOW_SPLASH_VERSION));
	...
	public void Gen_populate(android.database.Cursor cursor, int[] columnIndices) {
		...
		if (columnIndices[GEN_ID_CONNECTION_ID] >= 0
				&& !cursor.isNull(columnIndices[GEN_ID_CONNECTION_ID])) {
			gen_CONNECTION_ID = cursor
					.getLong(columnIndices[GEN_ID_CONNECTION_ID]);
		}
		if (columnIndices[GEN_ID_SHOW_SPLASH_VERSION] >= 0
				&& !cursor.isNull(columnIndices[GEN_ID_SHOW_SPLASH_VERSION])) {
			gen_SHOW_SPLASH_VERSION = cursor
					.getLong(columnIndices[GEN_ID_SHOW_SPLASH_VERSION]);
		}
		if (columnIndices[GEN_ID_TEXT_INDEX] >= 0
				&& !cursor.isNull(columnIndices[GEN_ID_TEXT_INDEX])) {
	...
	public void Gen_populate(android.content.ContentValues values) {
		...
		gen_SHOW_SPLASH_VERSION = values
				.getAsLong(GEN_FIELD_SHOW_SPLASH_VERSION);
